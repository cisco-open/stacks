...
def main():
    """The main function called when running `stacks postinit`"""
    ...
    for module in helpers.json_read([modulesdir.joinpath("modules.json")]).get(
        "Modules", []
    ):
        # ignore root module
        if module["Key"] == "":
        ...
        ):  # use git if the module doesn't to keep track of initial state between runs
            repo = git.Repo.init(moduledir)
            repo.config_writer().set_value("user", "name", "Stacks").release()
            repo.config_writer().set_value(
                "user", "email", "stacks@example.com"
            ).release()
            repo.git.add(".")
