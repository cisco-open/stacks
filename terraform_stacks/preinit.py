...
def main():
    """The main function called when running `stacks preinit`"""
    ...
    instance = (
        layer_split[1] if len(layer_split) > 1 else None
    )  # the layer's instance's name, otherwise None
    # validate context
    assert basedir.exists()
    assert envdir.exists()

    # remove workdir to avoid interference between runs
    helpers.directory_remove(
        workdir,
        keep=[
    ...
    helpers.directory_copy(
        ...
        + [
            ...
            "*.layer.tfvars",
            pathlib.Path(
                os.getenv("PLANFILE", "default.tfplan")
            ).name,  # do not copy plan files (binary)
            pathlib.Path(
                os.getenv("SHOWFILE", "default.json")
            ).name,  # do not copy plan files (json)
        ],
    )
    ...
    variables_declared = [
        ...
        for variable in helpers.hcl2_read([workdir.joinpath("*.tf")]).get(
            "variable", []
        )
    ]
    for variable in {
        **variables,
        **helpers.hcl2_read([workdir.joinpath("*.tfvars")]),
    }.keys():  # also autodeclare variables in *.auto.tfvars files
    ...
    for path in envsdir.iterdir():
        ...
        try:
            ...
            provider_aws_append(
                environment["region"], environment["role_arn"], path.name
            )
            # inject all other environments' other regions' providers
            for region in regions:
                provider_aws_append(
                    region, environment["role_arn"], f"{path.name}_{region}"
                )
        except KeyError:
            continue
    ...
    helpers.json_write(
        variables, workdir.joinpath("zzz.auto.tfvars.json")
    )  # 'zzz' so that it has the topmost precedence
